...
$stmt = $connect->prepare("SELECT password FROM worker WHERE id = ? LIMIT 1");
...
try {
    ...
    $update = $connect->prepare("UPDATE worker SET password = ? WHERE id = ?");
    $update->bind_param("si", $hashed, $worker_id);
    $update->execute();
