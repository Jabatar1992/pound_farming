...
if (!isset($_POST['id'])) {
    respondBadRequest("Availability ID is required.");
}
...
try {
    ...
    $stmt = $connect->prepare("DELETE FROM egg_availability WHERE id = ?");
    ...
    if ($stmt->affected_rows > 0) {
        $connect->commit();
        $stmt->close();
        respondOK("Egg availability deleted successfully.", []);
    } else {
        $connect->rollback();
        $stmt->close();
        respondBadRequest("Failed to delete egg availability.");
    }
