...
if (!isset($_POST['id'])) {
    respondBadRequest("Egg production ID is required.");
}
...
try {
    ...
    $stmt = $connect->prepare("DELETE FROM egg_production WHERE id = ?");
    ...
    if ($stmt->affected_rows > 0) {
        $connect->commit();
        $stmt->close();
        respondOK("Egg production record deleted successfully.", []);
    } else {
        $connect->rollback();
        $stmt->close();
        respondBadRequest("Failed to delete egg production record.");
    }
