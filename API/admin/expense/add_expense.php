...
if (!isset($_POST['category'], $_POST['description'], $_POST['amount'], $_POST['expense_date'], $_POST['recorded_by'])) {
    respondBadRequest("Invalid request. category, description, amount, expense_date and recorded_by are required.");
}
...
if (!in_array($category, $validCategories)) {
    ...
    try {
        ...
        $stmt = $connect->prepare("INSERT INTO expense (category, description, amount, expense_date, recorded_by) VALUES (?, ?, ?, ?, ?)");
        ...
        if ($stmt->affected_rows > 0) {
            ...
            respondOK("Expense recorded successfully.", $data);
        } else {
            $connect->rollback();
            $stmt->close();
            respondBadRequest("Failed to record expense.");
        }
